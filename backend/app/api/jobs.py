...
from app.services.analysis_service import AnalysisService, get_result, list_user_analyses
...
async def analyze_job_file(
    ...
    try:
        ...
        job_id = await service.analyze_job_file(
...
async def analyze_job(
    ...
    try:
        service = AnalysisService()
        job_id = await service.analyze_job_text(analyze_request.text, user_id=current_user)
        return AnalyzeJobResponse(job_id=job_id)
    except Exception as e:
        import logging
...
async def get_analysis_history(
    ...
    analyses = list_user_analyses(user_id=current_user)
...
async def get_historical_intelligence(
    ...
    '''Get historical intelligence for a job analysis (frontend compatibility).'''
    result = get_result(job_id)
    if not result:
    ...
    # Return mock historical intelligence for now
    # In a full implementation, this would query a vector database or similar
...
async def get_verification(
    ...
    '''Get verification data for a job analysis (frontend compatibility).'''
    result = get_result(job_id)
    if not result:
        raise HTTPException(status_code=404, detail='Analysis not found')
    ...
    if company and company.get('domain'):
        ...
        checks.append({
            'check_type': 'COMPANY_DOMAIN',
            'result': company.get('verification_status', 'UNVERIFIED'),
    ...
    if recruiter and recruiter.get('email'):
        ...
        checks.append({
            'check_type': 'RECRUITER_EMAIL_DOMAIN',
            'result': recruiter.get('verification_status', 'UNVERIFIED'),
...
async def recheck_verification(
    ...
    '''Re-run verification checks (frontend compatibility).'''
    result = get_result(job_id)
    if not result:
        raise HTTPException(status_code=404, detail='Analysis not found')
    ...
    # Return same verification data (in a real implementation, this would re-run checks)
    return await get_verification(job_id, current_user)
